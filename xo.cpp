...
public:
  ...
  int HaveWinner(void);     // dar soorati ke bazi barande dashte bashad, aan ra
                            // moshakh'khas mikonad
...
TicTaToe::TicTaToe(void) {
  for (int i = 0; i < 3;
       i++) // mogheye shoroo'e bazi, hameye safhe ra 0 (khaali) mikonim
    for (int j = 0; j < 3; j++)
      cell[i][j] = 0;
...
int TicTaToe::Play(int x) {
  x--;
  if (x >= 0 && x <= 8 &&
      (!cell[x / 3][x % 3])) { // agar taraf baziye ghanooni karde
  ...
  ShowBoard(); // taraf baziye gheyre mojaz karde ! safhe ra neshan midahim ta
               // dobare bazi konad
  return 0;
}
int TicTaToe::HaveWinner(void) {
  // in tabe moshakhas mikonad kasi barande shode ya na. be sadegi ba 2 ta for
  // satr o sotoon ra check mikonad va ba 2 ta if, zarbdari ha ra
  for (int i = 0; i < 3; i++)
    if (cell[i][0] != 0 && cell[i][0] == cell[i][1] &&
        cell[i][1] == cell[i][2] && cell[i][2] == cell[i][0])
      return cell[i][0]; // barande ra bar migardanim
  for (int i = 0; i < 3; i++)
    if (cell[0][i] != 0 && cell[0][i] == cell[1][i] &&
        cell[2][i] == cell[1][i] && cell[0][i] == cell[2][i])
  ...
  // agar kasi barande nashode bashad va hameye khaane ha por bashad yani mosavi
  // shode'and
  int tie = -1;
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
...
void TicTaToe::SetTurn(int player) { turn = player; }
char TicTaToe::GetTurn() { return (turn == 1 ? 'X' : 'O'); }
void TicTaToe::ShowBoard(
    void) // har vaght seda zade shava safheye bazi ra namayesh midahad
{
  cout << "\n";
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++)
...
int main() {
  ...
  while (!board.HaveWinner()) {
    cout << "Player " << board.GetTurn()
         << " : "; // bazi ra mikhanim va anjam midahim
    cin >> x;
  ...
  if (board.HaveWinner() > 0) // agar barande dashte migooyim kist
    cout << "\n\nWoww... the winner is "
         << (board.HaveWinner() == 1 ? "X" : "O") << "\n";
  else
    cout << "\n\nA tie ! \n"; // dar gheyre in soorat yani moasvi shode

  while (!kbhit())
    ;       // montazer mimanim yek kelid feshorde sahvad
